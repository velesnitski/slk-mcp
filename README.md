...
| `read_canvas` | Read canvases in a channel/DM (tab + shared); `date`/`match` select per-meeting notes, `list_only` lists them |
...
| `read_document` | Read text attachments (HTML, Markdown, TXT, CSV, JSON, source) inline; HTML is flattened to plain text. PDFs are fetched and returned as a local path |
